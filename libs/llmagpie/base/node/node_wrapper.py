...
class MakeNode:
    ...
    @classmethod
    def from_class(cls, class_: Optional[Type]=None, func_name: str="", outputs: Dict={}):
    ...
    @classmethod
    def from_function(cls, func: Optional[Callable]=None, name: Optional[str]=None, outputs: Dict={}):
...
def from_class(class_: Optional[Type]=None, func_name: str="", outputs: Dict={}):
    def _cls_wrapper(class_, func_name: str, outputs: Dict):
        assert func_name not in ["run", "stream", "async_run", "async_stream", "_stream", "async_call_",], \
            "The function name has conflict with the default function name."
        assert func_name and hasattr(class_, func_name), ValueError("function name is wrong.")
...
def from_function(func: Optional[Callable]=None, name: Optional[str]=None, outputs: Dict={}):
    def _make_with_name(_name) -> Callable:
        def _make_node(func: Callable) -> BaseModel:
            args = getfullargspec(func)
            if args.varargs or args.varkw:
