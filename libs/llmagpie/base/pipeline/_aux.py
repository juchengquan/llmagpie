...
def make_as_task(iterator: AsyncIterator | Coroutine, loop: AbstractEventLoop) -> Task:
    return (
        _as_task(iterator, loop)
        if isinstance(iterator, AsyncIterator)
        else loop.create_task(iterator)
    )
    ...
    for k, v in dt.items():
        if v is not None:
            if v[-1]["_type"] == "Pipeline":
                ...
                res.update({f"{k}.{kk}": vv for kk, vv in v[-1]["value"].items()})
    return res
