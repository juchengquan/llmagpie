...
def class_as_node(cls: Optional[Type]=None, func_name: str="", outputs: Dict={}):
    def _cls_wrapper(cls, func_name: str, outputs: Dict):
        ...
        assert func_name and hasattr(cls, func_name), ValueError("function name is wrong.")
        func_callable = getattr(cls, func_name)
        ...
        setattr(cls, "async_call_", _wrapper)
        setattr(cls, "input_model_schema", input_model)
        setattr(cls, "output_model_schema", output_model)

        return cls
    
    if cls:
        return _cls_wrapper(cls, func_name, outputs)
    return partial(_cls_wrapper, func_name=func_name, outputs=outputs)
        

def function_as_node(func: Optional[Callable]=None, name: Optional[str]=None, outputs: Dict={}):
