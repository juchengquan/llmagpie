...
def marshal_iterable(res_iterable: Generator, model: type[BaseModel]) -> Generator:
...
async def async_marshal_iterable(async_res_iterable: AsyncGenerator, model: type[BaseModel]) -> AsyncGenerator:
...
def post_run(res: Union[Tuple, Dict, Generator, AsyncGenerator], model: type[BaseModel]) \
    ...
    if isinstance(res, Generator):
        return marshal_iterable(res, model)
    elif isinstance(res, AsyncGenerator):
        return async_marshal_iterable(res, model)
